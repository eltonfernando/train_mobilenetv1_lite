...
def SeperableConv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, onnx_compatible=False):
    ...
    return Sequential(
        Conv2d(
            in_channels=in_channels, out_channels=in_channels, kernel_size=kernel_size, groups=in_channels, stride=stride, padding=padding
        ),
...
def create_mobilenetv3_large_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    return SSD(
        num_classes, base_net, source_layer_indexes, extras, classification_headers, regression_headers, is_test=is_test, config=config
    )
...
def create_mobilenetv3_small_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    regression_headers = ModuleList(
        [
            SeperableConv2d(in_channels=round(48 * width_mult), out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
